from sage.all import factorial, multinomial, Partitions, prod
...
def comb_partition_counts(n, multiplied=False, rep=False, verbose=True):
    if verbose:
    ...
    for i, p_list in enumerate(comb_pp):
        ...
        if multiplied:
            ...
            counts = [tuple([(n-sum(p))]*m) for p, m in zip(p_list, multinoms)]
        else:
            counts = [(n-sum(p), m) for p, m in zip(p_list, multinoms)]
